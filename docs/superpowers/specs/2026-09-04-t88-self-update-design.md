...
- **T86** — signing the release artifacts in CI, the Actions secrets, and the workflow that
  publishes `latest.json` to GitHub Releases. What this task takes from T86 is the *key*, because a
  pinned public key that is not a real key is a verification path nothing can exercise (D2).
...
**Generated here rather than in T86**, which owns "generation, CI signing of artifacts, pubkey
pinned in the app", because a constant that is not a valid minisign key makes every test in this
task untestable and the production path unbuildable. What T86 keeps is the half that has to happen
in CI: the secrets, the signing step, and the workflow that publishes `latest.json` and the payload
to a GitHub release. The roadmap entry is amended to say so rather than left to be discovered.
