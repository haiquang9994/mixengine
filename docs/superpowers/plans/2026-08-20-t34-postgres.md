...
Open an **elevated** PowerShell (Run as administrator). Confirm the token first, then ask:

```powershell
whoami /groups | Select-String -SimpleMatch 'S-1-5-32-544'   # must say "Enabled group"
$pg = "$env:LOCALAPPDATA\Temp\pg"
& "$pg\bin\initdb.exe" --pgdata "$pg\data" --username postgres --auth-local=scram-sha-256 --auth-host=scram-sha-256 --encoding=UTF8 --locale=C
...
Conditional on Task 1 Step 2. **If `postgres --single` ran happily under an elevated token, skip this
task entirely** and say so in the roadmap entry — the ritual's second step is a one-shot, and if it
does not care, neither does this path.
...
```rust
    ...
    fn the_cluster_is_created_with_a_locale_this_recipe_chose() {
        ...
        assert!(
            initdb.args.iter().any(|arg| arg == "--auth-local=scram-sha-256"),
            "{:?}",
            initdb.args
        );
        assert!(
            initdb.args.iter().any(|arg| arg == "--auth-host=scram-sha-256"),
            "{:?}",
            initdb.args
        );
    }
...
**One thing this plan cannot promise.** Task 4 is conditional on Task 1 Step 2, and Task 10's
`service set … extra` / `service reload` spellings are to be confirmed against the CLI before the
suite is run. Both are marked at the step rather than left to be discovered.
