-- What `schema-0015.db` was seeded with — roadmap task T89.
--
-- The last schema before the two table rebuilds. Its sites, domains and links are carried across by
-- `0016` and `0017`, which is what makes this the fixture that proves those two rebuilds keep what
-- they found — and the one the CLI test drives, because it is the only one whose sites are still
-- there afterwards.
--
-- Nothing in it is real; see the note at the top of `schema-0001.sql`.
--
...
-- rule, which is not something a fixture should provoke. `schema-0017.sql` carries both, and
-- nothing starts a daemon on that one.
...
VALUES
    ('mariadb@main', 1, NULL, 'main', 'stopped', 1, 3306, 13306, '127.0.0.1',
     '/home/dev/MixEngine/data/mariadb/main', '{"innodb_buffer_pool_size":"256M"}', '{}', 30, 1,
     1767517200000, 0, NULL, NULL),
    ('caddy@main',   2, NULL, 'main', 'stopped', 1,   80, NULL, '127.0.0.1',
     NULL, '{}', '{}', NULL, 0, 1767517260000, 0, NULL, NULL),
    ('php-fpm@8.3',  NULL, 1, '8.3',  'stopped', 0, 9000, 19000, '127.0.0.1',
     NULL, '{}', '{"memory_mb":512}', 10, 0, NULL, NULL, NULL, NULL);

-- `trusted` and `signature` as 0014 and 0015 leave them, written here rather than defaulted so the
-- census has values to carry rather than defaults to re-derive.
...
-- The shape 0016 replaces. Seeded anyway: EMPTIED says this row is lost at 16, and
-- `the_tables_two_migrations_empty_really_are_emptied` fails if the fixture has nothing to lose.
...
-- 0007: an operation waiting for a prompt nobody has answered yet.
INSERT INTO pending_privileged_ops (id, op, dedupe_key, requested_at)
VALUES (1, 'hosts.apply', 'hosts:blog.test', 1767517400000);

-- 0011: a WITHOUT ROWID table, which is the one shape in this schema a rebuild could get wrong.
