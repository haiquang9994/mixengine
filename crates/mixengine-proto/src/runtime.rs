...
//!
//! T24 added [`VersionConstraint`] and [`RuntimeVersion::cmp_precedence`] beside the identifier they
...
impl RuntimeVersion {
    ...
    /// make `BTreeMap<RuntimeVersion, _>` silently change what it keys on, and a comparison this
...
impl VersionConstraint {
    ...
    pub const MAX_LEN: usize = RuntimeVersion::MAX_LEN + 1;

    /// Read a constraint, refusing anything that is not one of the three forms.
    ///
    /// The value after any `^` is held to [`RuntimeVersion::parse`]'s own rules rather than to
    ...
    pub fn parse(value: impl Into<String>) -> Result<Self, VersionError> {
        ...
        RuntimeVersion::parse(bare).map_err(|error| VersionError {
            value: value.clone(),
            reason: error.reason,
        })?;
    ...
    #[must_use]
    pub fn exact(&self) -> Option<RuntimeVersion> {
        ...
        RuntimeVersion::parse(self.0.clone()).ok()
    }
...
impl From<RuntimeVersion> for VersionConstraint {
...
impl<'de> serde::Deserialize<'de> for VersionConstraint {
    /// Validating, for [`RuntimeVersion`]'s reason one step further out: a range that is not one is
...
mod tests {
    ...
    fn a_version_is_newer_than_another_the_way_upstream_means_it() {
        let newest = |versions: [&str; 2]| {
            let [left, right] = versions.map(|text| RuntimeVersion::parse(text).expect(text));
        ...
        let (short, padded) = (
            RuntimeVersion::parse("8.3").expect("a version"),
            RuntimeVersion::parse("8.3.0").expect("a version"),
        );
        assert_eq!(
        ...
        let (plain, built) = (
            RuntimeVersion::parse("1.2.3").expect("a version"),
            RuntimeVersion::parse("1.2.3+build4").expect("a version"),
        );
    ...
    fn a_constraint_selects_the_versions_it_names_and_no_others() {
        for (asked, version, expected) in [
            ...
            let version = RuntimeVersion::parse(version).expect(version);
            assert_eq!(
