...
| [3 — Services](phase-3-services.md) | Web server, databases and caches with generated config | T30–T38 | 12 / 15 | **M3** caddy + mariadb + redis healthy in under 10 s warm |
...
**Phase 3 is 12 of 15.** [T30](phase-3-services.md) is in, and with it the port T19 left open is
...
**The next database is a second product and not a second version.** MySQL is packed and in the
index — [T34b](phase-3-services.md), in
...
withdrew macOS from them while they were alive and never built ARM for either. The recipe that turns
one of those artifacts into a running service is [T34c](phase-3-services.md). Most of what it needs,
T33 already built: the ritual that stores a generated password before anything is created,
`ReadyCheck::Command`, and the pair of markers that lets a half-finished data directory be cleaned
without ever clearing somebody else's database. What is new is the bootstrap — three routes rather
than one, because 5.6 has no `--initialize-insecure` and Windows 5.6 has no installer at all — and
the fact that `--initialize-insecure` creates only `root@localhost`, so MariaDB's
`skip-name-resolve` cannot travel from one `my.cnf` to the other. Both want port 3306, so T34c is
also where a port stops being something a caller must name: a recipe declares the one it prefers,
the first row to ask is given it, the next is given the first free port above, and what is written
down is never recomputed — first created, first served, with the same allocator answering T36's two
instances of one recipe.
