...
- idle footprint < 60 MB RSS
  ...
  **median of five readings** taken thirty seconds after the last command, and asserts the set of
  subjects before the total: a home whose web server died reports one subject and a very good number,
  which is this measurement's failure that reads as a pass. The reading is the daemon's own
  `metrics.snapshot`, so what is gated is the number `mix metrics` shows a user rather than a second
  opinion taken beside it (T72)
